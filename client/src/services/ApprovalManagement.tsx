              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredApplications.map((application) => (
                  <tr key={application._id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-medium text-gray-900">
                            {application.userId?.personalInfo?.name || 'Unknown'}
                          </div>
                          <div className="text-sm text-gray-500">
                            {application.userId?.personalInfo?.email || 'No email'}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {getTypeIcon(application.userId?.role || 'user')}
                        <span className="ml-2 text-sm text-gray-900 capitalize">
                          {application.userId?.role || 'user'}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">
                        {application.userId?.role === 'factory' ? (
                          <>
                            <div className="font-medium">{application.factoryName || 'No factory name'}</div>
                            <div className="text-gray-500 text-xs">GST: {application.gstNumber || 'N/A'}</div>
                          </>
                        ) : (
                          <>
                            <div className="font-medium">{application.companyName || 'No company name'}</div>
                            <div className="text-gray-500 text-xs">
                              {application.serviceArea?.join(', ') || 'No service area'}
                            </div>
                          </>
                        )}
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      {application.status === 'pending' ? (
                        <div className="flex flex-col gap-2">
                          <button
                            onClick={() => handleApprove(application._id, application.userId?.role as 'factory' | 'collector' || 'user')}
                          ...
                          <div className="mt-2">
                            ...
                            />
                            <button
                              onClick={() => handleReject(application._id, application.userId?.role as 'factory' | 'collector' || 'user')}
